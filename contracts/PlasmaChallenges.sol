...
// TODO either account in here OR in global UTXO challenge procedure
...
// no longer valid with padded blocks
...
// Only operator have a power for such merges
    function proveBalanceOrOwnershipBreakingBetweenInputAndOutput(
        ...
        uint256 _inputOfInterest)
         public returns(bool success) {
        uint256 idx = 0;
        BankexPlasmaTransaction.PlasmaTransaction memory spendingTX = BankexPlasmaTransaction.signedPlasmaTransactionFromBytes(_plasmaTransaction);
        BankexPlasmaTransaction.PlasmaTransaction memory originatingTX = BankexPlasmaTransaction.signedPlasmaTransactionFromBytes(_originatingPlasmaTransaction);
        (success, idx) = BankexPlasmaTransaction.checkForInclusionIntoBlock(blockStorage.getMerkleRoot(_originatingPlasmaBlockNumber), _originatingPlasmaTransaction, _originatingMerkleProof);
        require(success);
        originatingTX.txNumberInBlock = uint32(idx);
        (success, idx) = BankexPlasmaTransaction.checkForInclusionIntoBlock(blockStorage.getMerkleRoot(_plasmaBlockNumber), _plasmaTransaction, _merkleProof);
        require(success);
    ...
    function checkRightfulInputOwnershipAndBalance(
        BankexPlasmaTransaction.PlasmaTransaction memory _spendingTX,
        BankexPlasmaTransaction.PlasmaTransaction memory _originatingTX,
...
// Convenience functions
    ...
    function getTXinputDetailsFromProof(
        ...
        bytes _plasmaTransaction,
        bytes _merkleProof) 
        internal view returns (address signer, BankexPlasmaTransaction.TransactionInput memory input, uint256 index) {
