...
library BankexPlasmaTransaction {
    ...
    function checkForInclusionIntoBlock(bytes32 _merkleRoot, bytes _plasmaTransaction, bytes _merkleProof) internal pure returns (bool included, uint256 txNumber) {
        (included, txNumber) = checkProof(_merkleRoot, _plasmaTransaction, _merkleProof, true);
        return (included, txNumber);
    }

    function checkProof(bytes32 root, bytes data, bytes proof, bool convertToMessageHash) internal pure returns (bool included, uint256 txNumber) {
        bytes32 h;
